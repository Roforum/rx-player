...
import { IParsedRepresentation } from "../../dash/manifest/node_parsers/Representation";
...
export default function patchSegmentsIndex(period: IParsedPeriod) {
    ...
    adapt.forEach((adaptation: IParsedAdaptationSet) => {
      ...
      reps.forEach((rep: IParsedRepresentation) => {
        const index = rep.index;
        if(index && index.getTimeOffset() == null){
          index.setTimeOffset(period.start);
          const timeline = index.getTimeline();
          const timescale = index.getTimescale() || 0;
          if(timeline != null && period.start != null){
            timeline.forEach((tl: any) => {
              tl.ts += ((index.getTimeOffset() || 0) * timescale);
          });
          }
        }
      });
